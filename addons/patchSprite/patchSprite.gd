...
func _draw_rect(p_rect:Rect2, p_sourceRect:Rect2, p_xMode:int, p_yMode:int):
	if normalMap:
		VisualServer.canvas_item_add_nine_patch(get_canvas_item(), p_rect, p_sourceRect, texture.get_rid(),
				 Vector2.ZERO, Vector2.ZERO, p_xMode, p_yMode, true, Color.white, normalMap.get_rid())
	else:
		VisualServer.canvas_item_add_nine_patch(get_canvas_item(), p_rect, p_sourceRect, texture.get_rid(),
				 Vector2.ZERO, Vector2.ZERO, p_xMode, p_yMode)

func _draw_patch_container(p_patchContainer, p_rect:Rect2, p_sourceRect:Rect2):
	var avail:float
	var scalePatches := []
	var drawPatches := []
	var totalRatio := 0.0
	...
	if p_patchContainer is HPatchContainer:
		axis = "x"
		childRect.size.y = p_rect.size.y
	else:
		axis = "y"
		childRect.size.x = p_rect.size.x
	avail = p_rect.size[axis] - p_patchContainer.minSize[axis]
	
	for i in p_patchContainer.children:
	...
	for i in drawPatches:
		...
		i.rect = childRect
		if i.rect.size[axis] <= 0.0 || reg == null || not p_sourceRect.intersects(reg):
			continue
		
		if i is Patch:
			if is_using_isometric():
				_draw_isometric(childRect, p_sourceRect.clip(reg), i.xMode, i.yMode)
			else:
				_draw_rect(childRect, p_sourceRect.clip(reg), i.xMode, i.yMode)
		else:
			_draw_patch_container(i, childRect, p_sourceRect)
...
func _clip_isometric_uvs(p_points:PoolVector2Array, p_uvs:PoolVector2Array):
	if p_uvs[3].x < 0.0 || p_uvs[0].y < 0.0 || p_uvs[1].x > texture.get_size().x || p_uvs[2].y > texture.get_size().y:
		var texPoints = [ Vector2.ZERO, Vector2(texture.get_size().x, 0.0), texture.get_size(), Vector2(0.0, texture.get_size().y)]
		var r_uvs = Geometry.intersect_polygons_2d(texPoints, p_uvs)
		if r_uvs.size() <= 0:
			return [ PoolVector2Array(), PoolVector2Array() ]
		r_uvs = r_uvs[0]
		var uvScale = Vector2((p_points[1].x - p_points[0].x) / (p_uvs[1].x - p_uvs[0].x), (p_points[3].x - p_points[0].x) / (p_uvs[3].x - p_uvs[0].x))
		
		var r_points := PoolVector2Array()
		r_points.resize(r_uvs.size())
		var uv
		for i in r_uvs.size():
			uv = _world_to_isometric(r_uvs[i] - p_uvs[0]) * uvScale
			r_points[i] = p_points[0] + uv.x * isometricNormal[0] + uv.y * isometricNormal[1]
		
		return [r_points, r_uvs]
	
	return [p_points, p_uvs]

func _draw_isometric(p_rect:Rect2, p_sourceRect:Rect2, p_xMode:int, p_yMode:int):
	p_rect.position = isometricNormal[0] * p_rect.position.x + isometricNormal[1] * p_rect.position.y
	p_sourceRect.position = isometricNormal[0] * p_sourceRect.position.x + isometricNormal[1] * p_sourceRect.position.y
	var uvs = _isometric_rect_to_points(p_sourceRect)
	var points = _isometric_rect_to_points(p_rect)
	var clipResult = _clip_isometric_uvs(points, uvs)
	if clipResult[0].size() < 3:
		return
	...
	for i in uvs.size():
		uvs[i] = uvs[i] / texture.get_size()

	if p_xMode == MODE_STRETCH && p_yMode == MODE_STRETCH:
		if normalMap:
			VisualServer.canvas_item_add_polygon(get_canvas_item(), points, [ Color.white ], uvs, texture.get_rid(), normalMap.get_rid())
		else:
			VisualServer.canvas_item_add_polygon(get_canvas_item(), points, [ Color.white ], uvs, texture.get_rid())
	
	elif p_xMode != MODE_STRETCH && p_yMode != MODE_STRETCH:
		var xTileCount:int
		var yTileCount:int
		var tileW:float
		var tileH:float
		if p_xMode == MODE_TILE:
			xTileCount = int(ceil(p_rect.size.x / p_sourceRect.size.x))
			tileW = p_sourceRect.size.x
		else:
			xTileCount = int(max(round(p_rect.size.x / p_sourceRect.size.x), 1.0))
			tileW = p_rect.size.x / float(xTileCount)
		
		if p_yMode == MODE_TILE:
			yTileCount = int(ceil(p_rect.size.y / p_sourceRect.size.y))
			tileH = p_sourceRect.size.y
		else:
			yTileCount = int(max(round(p_rect.size.y / p_sourceRect.size.y), 1.0))
			tileH = p_rect.size.y / float(yTileCount)
		
		var tilePoints = _isometric_rect_to_points(Rect2(p_rect.position, Vector2(tileW, tileH)))
		var xVec = isometricNormal[0] * tileW
		var yVec = isometricNormal[1] * tileH
		var xLoopCount
		var yLoopCount
		if p_xMode == MODE_TILE:
			xLoopCount = xTileCount - 1
		else:
			xLoopCount = xTileCount
		
		if p_yMode == MODE_TILE:
			yLoopCount = yTileCount - 1
		else:
			yLoopCount = yTileCount
		
		for x in xLoopCount:
			for y in yLoopCount:
				if normalMap:
					VisualServer.canvas_item_add_polygon(get_canvas_item(), tilePoints, [ Color.white ], uvs, texture.get_rid(), normalMap.get_rid())
				else:
					VisualServer.canvas_item_add_polygon(get_canvas_item(), tilePoints, [ Color.white ], uvs, texture.get_rid())
				
				for i in tilePoints.size():
					tilePoints[i] += yVec
			
			for i in tilePoints.size():
				tilePoints[i] += -yVec * yLoopCount + xVec
		
		if p_xMode == MODE_TILE:
			var temp = PoolVector2Array(tilePoints)
			var w = p_rect.size.x - xLoopCount * tileW
			for i in temp.size():
				temp[i] += -yVec * yLoopCount
			
			var diff = xVec.normalized() * (w - tileW)
			temp[1] += diff
			temp[2] += diff
			
			var newUVs = _isometric_rect_to_points(Rect2(p_sourceRect.position, Vector2(w, p_sourceRect.size.y)))
			for i in newUVs.size():
				newUVs[i] = newUVs[i] / texture.get_size()
			
			for y in yLoopCount:
				if normalMap:
					VisualServer.canvas_item_add_polygon(get_canvas_item(), temp, [ Color.white ], newUVs, texture.get_rid(), normalMap.get_rid())
				else:
					VisualServer.canvas_item_add_polygon(get_canvas_item(), temp, [ Color.white ], newUVs, texture.get_rid())
				
				for i in temp.size():
					temp[i] += yVec
			
			if p_yMode == MODE_TILE:
				var h = p_rect.size.y - yLoopCount * tileH
				var diff2 = yVec.normalized() * (h - tileH)
				temp[2] += diff2
				temp[3] += diff2
				newUVs[2] +=  diff2
				newUVs[3] += diff2
				...
				for x in xLoopCount:
					...
					for i in temp.size():
						temp[i] += xVec
		elif p_yMode == MODE_TILE:
			var temp = PoolVector2Array(tilePoints)
			var h = p_rect.size.y - yLoopCount * tileH
			for i in temp.size():
				temp[i] += -xVec * xLoopCount
			
			var diff = yVec.normalized() * (h - tileH)
			temp[2] += diff
			temp[3] += diff
			
			var newUVs = _isometric_rect_to_points(Rect2(p_sourceRect.position, Vector2(p_sourceRect.size.x, h)))
			for i in newUVs.size():
				newUVs[i] = newUVs[i] / texture.get_size()
		...
		var tilePoints = _isometric_rect_to_points(Rect2(p_rect.position, Vector2(p_rect.size.x, tileH)))
		var yVec = isometricNormal[1] * tileH
		var yLoopCount
		...
		var tilePoints = _isometric_rect_to_points(Rect2(p_rect.position, Vector2(tileW, p_rect.size.y)))
		var xVec = isometricNormal[0] * tileW
		var xLoopCount
